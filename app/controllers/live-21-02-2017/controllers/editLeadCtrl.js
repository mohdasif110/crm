...
//		});
		...
		*/
		 ...
		 */
		$scope.bhk1 = [];
		$scope.setBHK = function ( bhk_value , e) {
			
			var element_check_state = angular.element(e.currentTarget).prop('checked');
			if(element_check_state){
				$scope.bhk1.push(bhk_value);
			}else{
				var index_of_value = $scope.bhk1.indexOf (bhk_value);
				if(index_of_value > -1){
					$scope.bhk1.splice (index_of_value,1);
				}
			}	
