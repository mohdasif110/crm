...
( function ( Pace, $ ) {
	...
	app = angular.module ( 'app', ['ngRoute', 'ngSanitize', 'ngAnimate', 'ui.bootstrap','ui.router','uiSwitch'] );
	...
	app.value ( 'appUrls', {
		...
		stuffUrl: 'http://localhost/CRM/stuff/',
		appUrl: 'http://localhost/CRM/app/',
		apiUrl: 'http://localhost/CRM/apis/'
	} );
	...
	app.constant ('baseUrl', 'http://localhost/CRM/');
		...
		when('/manage_disposition_group_status/:group/:group_id/:employee_id', {
				...
				when('/add_sales_person_capacity/:sales_person_id/:manager_id/:manager_capacity/:mode',{
					...
					resolve : {
						...
						asm_mpl_capacity : function (httpService, $route){
							
							var asm_id				= $route.current.params.manager_id;
							
							return httpService.makeRequest({
								url : baseUrl + 'apis/helper.php?method=get_mpl_capacity&params=user_id:'+asm_id,
								method : 'GET',
								data : {}
							}).then(function (responseData){
								return responseData.data.capacity;
							});
						}
					}
				}).	
