...
$lead_status_type_slug		= 'technical_issue';
$technical_issue_leads		= array();
$parent_status_id			= '';
...
if(isset($parent_status_id) && $child_status_id){
	...
	$select_leads = 'SELECT '
			...
			. '	customerEmail, customerCity, disposition_status_id, disposition_sub_status_id '
			. ' FROM '
			. '	`lead` '
			. ' WHERE assignedTo = '.$user_id.' AND disposition_status_id = '.$parent_status_id.' AND disposition_sub_status_id IN( '.implode(',',$child_status_id).')';
	...
	if($leads_result && mysql_num_rows($leads_result) > 0){
		...
		while($row = mysql_fetch_assoc($leads_result)){
			array_push($technical_issue_leads, $row);
		}
		
	}
}

echo json_encode($technical_issue_leads, true);