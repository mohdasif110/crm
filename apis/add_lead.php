...
if (isset($_POST)) {
	...
	if ( isset($_POST['enquiry']['id']) && !empty($_POST['enquiry']['id']) ) {
		...
		if( strtolower( $lead_enquiry_status_title ) === 'not_interested'){
			
			$enquiry_sub_status_id	= $_POST['enquiry']['sub_status_id'];
			$enquiry_remark			= $_POST['enquiry']['status_remark'];
			
			if(!empty($_POST['enquiry']['status_remark'])){
				$form_errors['status_remark'] = 'Please fill status remark';
			}else{
				$enquiry_remark = $_POST['enquiry']['status_remark'];
			}
			
		} // end of condition to check not_intrested primary status 
		...
		// for rest of the primary enquiry status and sub status 
				...
				if($enquiry_sub_status_id == ''){
					...
					switch( strtolower( $lead_enquiry_status_title) ){
	...
	if (!empty($form_errors)) {
		...
		if (mysql_query($insert_lead_sql)) {
			...
			$lead_details = 'Lead has been added by '.$user_name;
			$lead_type = 'new';
			...
			if (strtolower($lead_enquiry_status_title) === 'meeting') {
				...
				$insert_meeting_data = 'INSERT INTO `lead_meeting` (enquiry_id,meeting_status,employee_id,meeting_date,meeting_time,meeting_address,remark)'
						. ' VALUES (' . $enquiry_id . ',"' . $enquiry_sub_status_title . '","' . $user_id . '","' . date('Y-m-d', strtotime($callback_date)) . '","' . $callback_time . '","'.$meeting_or_site_visit_address.'","' . $callback_status_remark . '") ';
				
				mysql_query($insert_meeting_data);
				
			} else if (strtolower($lead_enquiry_status_title) === 'site_visit') {

				$insert_site_visit_data = 'INSERT INTO `site_visit` (enquiry_id, site_visit_status, site_visit_date, site_visit_time, employee_id, client_name, client_email, client_number, remark, site_location) '
						. ' VALUES (' . $enquiry_id . ',"' . $enquiry_sub_status_title . '","' . date('Y-m-d', strtotime($callback_date)) . '","' . $callback_time . '", ' . $user_id . ', "' . $client_name . '","' . $client_email . '","' . $client_number . '","' . $enquiry_status_remark . '","'.$meeting_or_site_visit_address.'")';
			...
			echo json_encode(array('success' => 1, 'message' => 'Lead has been added successfully ', 'save_lead_history_status' => $lead_history_save_status, 'save_enquiry_projects_flag' => $flag_of_save_enquiry_projects), TRUE);
			exit;
		} else {
	...
	echo json_encode(array('success' => 0, 'message' => 'No data received'), TRUE);
	exit;
}